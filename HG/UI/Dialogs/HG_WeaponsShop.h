...
class HG_WeaponsShop
{
	...
	class Controls
	{
		...
		class MyCashButton: HG_RscButtonInvisible
		{
			...
			onButtonClick = "hint format[(localize 'STR_HG_DLG_MC'),if((getNumber(missionConfigFile >> 'CfgClient' >> 'enableSave')) isEqualTo 1) then {[(profileNamespace getVariable 'HG_Save'),true] call HG_fnc_currencyToText} else {[(player getVariable 'HG_myCash'),true] call Client_fnc_currencyToText}]";
			x = 30.5 * GUI_GRID_W + GUI_GRID_X;
			y = 2.5 * GUI_GRID_H + GUI_GRID_Y;
			w = 3 * GUI_GRID_W;
			h = 2 * GUI_GRID_H;
