...
if((uniform _unit) != "") then 
{
	...
	_uWeapList = getWeaponCargo uniformContainer _unit;
	{
        _amntList = _uWeapList select 1;
        _tmp pushBack [_x,(_amntList select _forEachIndex)];
    } forEach (_uWeapList select 0);
...
if((vest _unit) != "") then 
{
	...
	_vWeapList = getWeaponCargo vestContainer _unit;
    {
        _amntList = _vWeapList select 1;
        _tmp pushBack [_x,(_amntList select _forEachIndex)];
    } forEach (_vWeapList select 0);
...
if((backpack _unit) != "") then 
{
	...
	_bWeapList = getWeaponCargo backpackContainer _unit;
    {
        _amntList = _bWeapList select 1;
        _tmp pushBack [_x,(_amntList select _forEachIndex)];
    } forEach (_bWeapList select 0);
