...
switch(_mode) do
{
	...
	{
	    _price = _price * _qty;
	    private _discount = ((getNumber(missionConfigFile >> "CfgClient" >> "HG_MasterCfg" >> (rank player) >> "iShopDiscount")) != 0) AND (_price != 0);
		
		if(_discount) then
		{ 
		    _price = round(_price - (_price * ((getNumber(missionConfigFile >> "CfgClient" >> "HG_MasterCfg" >> (rank player) >> "iShopDiscount")) / 100)));
	    };
